...
tar_files_to_extract = 1


conn = sqlite3.connect("testv12.db")
...
with open('seenv12.txt', 'r') as file:
...
for b,a in enumerate(all_files_in_year_path_relevant):
    ...
    for x,i in enumerate(all_json_files):
        ...
        if not email_id:
            if not text_resume:
                print(f"file: {file_id} skipped because it didn't have both email id and text resume")
                continue
        ...
        try:
            ...
            print("Duplicate file, hence skipped")
            continue
    ...
    with open('seenv12.txt', 'a') as f:
        f.write("%s\n" % tarfile_name)
        # all_folders_completed.append(tarfile_name)

    print(f"{tarfile_name} Completed")
