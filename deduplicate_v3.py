...
for a in all_files_in_year_path_relevant:
    ...
    for a in glob(f"{folder_path}/*json_cv"):
        shutil.move(a, temp_folder)
    ...
    for x,i in enumerate(all_json_files):
        ...
        if old_file_email_len > 0:
            ...
            if date <= old_file_info[0].get("date"):
                print(f"Discarded {file_id} because it is older than {old_file_info[0].get('fileId')}")
                continue
                ...
                c.execute(f"""INSERT INTO file_info VALUES ('{file_id}', '{email_id}', '{date}', '{text_resume_hash}', '{given_family_code}')""")
                conn.commit()
